...
`lxdb --check-lldb-bindings` performs an explicit preflight for the LLDB Ruby bindings. The main GitHub Actions `LLDB integration` job runs that preflight on every push so broken LLDB Ruby bindings fail CI before integration specs run. Set `LXDB_REQUIRE_LLDB_BINDINGS=1` to make process-backed integration limitations fail instead of being reported as pending environment limitations. The manual `LLDB Ruby bindings` workflow keeps that stricter mode available for runners with a known-good LLDB process-debugging environment.
